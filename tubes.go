...
func sortUssername(A *acc, i *int) {
	var n int
	// Mengurutkan username untuk pengecekan cekRegis menggunakan insertion sort secara ascending
	for n = 1; n < *i; n++ {
		for j := n; j > 0; j-- {
			if (*A)[j-1].username > (*A)[j].username {
				temp := (*A)[j-1]
				(*A)[j-1] = (*A)[j]
				(*A)[j] = temp
			}
